...
### Model version 3
class CN1NGenerator(nn.Module):
	def __init__(self):
		super().__init__()
		self.avgPool2 = nn.AvgPool1d(2, ceil_mode=True)
		self.upsample2 = nn.Upsample(scale_factor=2, mode='linear')
		self.cnn1 = nn.Sequential(
                 	nn.Conv1d(1, 16, kernel_size=1, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(16),
                	nn.ReLU(True),
                )
		self.cnn2 = nn.Sequential(
                 	nn.Conv1d(16, 32, kernel_size=2, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(32),
                	nn.ReLU(True),
                )
		...
		self.cnn8 = nn.Sequential(
                 	nn.Conv1d(64, 128, kernel_size=8, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(128),
                	nn.ReLU(True),
                )
		...
		self.cnn16 = nn.Sequential(
                 	nn.Conv1d(128, 256, kernel_size=16, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(256),
                	nn.ReLU(True),
                )
		...
		self.cnn64 = nn.Sequential(
                 	nn.Conv1d(256, 256, kernel_size=64, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(256),
                	nn.ReLU(True),
                )
