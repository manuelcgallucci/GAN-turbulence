...
class DiscriminatorStructuresUNet(nn.Module):
	def __init__(self):
		super().__init__()
		self.avgPool2 = nn.AvgPool1d(2, ceil_mode=True)
		self.upsample2 = nn.Upsample(scale_factor=2, mode='linear')
		self.cnn1 = nn.Sequential(
                 	nn.Conv1d(1, 4, kernel_size=1, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(4),
                	nn.ReLU(True),
                )
		self.cnn2 = nn.Sequential(
                 	nn.Conv1d(4, 8, kernel_size=2, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(8),
                	nn.ReLU(True),
                )
		self.cnn2_ = nn.Sequential(
                 	nn.Conv1d(8, 16, kernel_size=2, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(16),
                	nn.ReLU(True),
                )
		self.cnn4 = nn.Sequential(
                 	nn.Conv1d(16, 32, kernel_size=4, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(32),
                	nn.ReLU(True),
                )
		self.cnn4_ = nn.Sequential(
                 	nn.Conv1d(32, 64, kernel_size=4, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(64),
                	nn.ReLU(True),
                )
		...
		self.bridge1 = nn.Sequential(
                 	nn.Conv1d(128, 128, kernel_size=32, stride=1, padding="same", bias=False),
                	nn.BatchNorm1d(128),
                	nn.ReLU(True),
                )
		self.cnnTrans8 = nn.Sequential(
                 	nn.Conv1d(128, 64, kernel_size=8, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(64),
                	nn.ReLU(True),
                )
		self.cnnTrans4 = nn.Sequential(
                 	nn.Conv1d(64, 32, kernel_size=4, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(32),
                	nn.ReLU(True),
                )
		self.cnnTrans2 = nn.Sequential(
                 	nn.Conv1d(32, 16, kernel_size=2, stride=1, padding=0, bias=False),
                	nn.BatchNorm1d(16),
                	nn.ReLU(True),
                )
...
class DiscriminatorMultiNet16_2(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        ...
        p2048_0 = self.cnn2048(x[:,:,:2048])
        p2048_1 = self.cnn2048(x[:,:,2048:4096])
        p2048_2 = self.cnn2048(x[:,:,4096:6144])
        p2048_3 = self.cnn2048(x[:,:,6144:8192])
        p2048_4 = self.cnn2048(x[:,:,8192:10240])
        p2048_5 = self.cnn2048(x[:,:,10240:12288])
        p2048_6 = self.cnn2048(x[:,:,12288:14336])
        p2048_7 = self.cnn2048(x[:,:,14336:16384])
        p2048_8 = self.cnn2048(x[:,:,16384:18432])
        p2048_9 = self.cnn2048(x[:,:,18432:20480])
        p2048_10 = self.cnn2048(x[:,:,20480:22528])
        p2048_11 = self.cnn2048(x[:,:,22528:24576])
        p2048_12 = self.cnn2048(x[:,:,24576:26624])
        p2048_13 = self.cnn2048(x[:,:,26624:28672])
        p2048_14 = self.cnn2048(x[:,:,28672:30720])
        p2048_15 = self.cnn2048(x[:,:,30720:])
        
        out = torch.cat((p16384_0, p16384_1, p8192_0, p8192_1, p8192_2, p8192_3, p4096_0, p4096_1, p4096_2, p4096_3, p4096_4, p4096_5, p4096_6, p4096_7, p2048_0, p2048_1, p2048_2, p2048_3, p2048_4, p2048_5, p2048_6, p2048_7, p2048_8, p2048_9, p2048_10, p2048_11, p2048_12, p2048_13, p2048_14, p2048_15),dim=1)
        return out
